...
#define MYPORT SERVERBPORT
#define PORT SERVERUDPPORT


using namespace std;

// the interval time msg to main server:  IB[[time,time],[time,time],[time,time]]
...
int udp_listener( vector<string>& list ){ // receive requested name list from main server using UDP
    ...
    for( int i=3 ; i<numbytes ; i++ ){   // convert the name list string to a list
...
string decorate( vector< vector<int>> data ){  // convert interval time from 2-D array to a string
    ...
    if( results.size() != 1 ) results.pop_back();  //vector
...
void getFile( unordered_map< string, vector< vector<int> >>& data){ // get input file and store it into a map
    ...
    file.open("input_files/b.txt");  //fstream
...
void printInput( unordered_map< string, vector< vector<int> >>& data ){
        ...
        if( !flag ){
            ...
            for( const auto& d_vt:data[name_vt] ){ // iterate all interval time mapped to the name <vector>
                ...
                for( int i=0 ; i<buf.size() ; i++  ){
                    ...
                    if( buf[i][0]>=d_vt[0] && buf[i][1]<=d_vt[1] )results.push_back( buf[i] );
                    else if( buf[i][0]<=d_vt[0] && buf[i][1]>=d_vt[1] ){ // if one interval time totally cover another
                        
                        if( d_vt[1]!= buf[i][1]) buf.push_back( { d_vt[1], buf[i][1] } );
...
int main(void) {
    ...
    cout<< "ServerB is up and running using UDP on port "<<SERVERBPORT<<"."<<endl;
    ...
    while(1){
        ...
        cout<<"Server B received the usernames from Main Server using UDP over port "<<SERVERBPORT<<"."<<endl;

        vector<vector<int>> interval_time;

        getInterval( requestedList, data, interval_time );

        string decorated_time = decorate( interval_time );

        cout<<"Found the intersection result:"<<decorated_time<<" for ";
        for( int i=0 ; i<requestedList.size() ; i++ ){

            cout<<requestedList[i];
            if( i!=requestedList.size()-1 ) cout<<", ";
            else cout<<"."<<endl;
        }

        decorated_time = "IB"+decorated_time;
